...
function Ta({ value, onChange, placeholder, rows = 3, noBracketBtn }: {
  ...
  // Re-scan brackets on every value change to drive the red-border
  // signal. The badge in DraggableQuestionList shares the same
  // scanner so the two signals stay consistent.
  ...
  const ta = (
    <textarea ref={ref} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      rows={rows} style={{
        ...taBase,
        borderColor: hasErr ? "rgba(239,68,68,0.8)" : (f ? C.accent : C.border2),
        ...(noBracketBtn ? {} : { flex: 1, minWidth: 0 }),
      }}
      onFocus={() => setF(true)} onBlur={() => setF(false)} />
  );
...
function RichTa({ value, onChange, placeholder, rows = 5 }: {
  ...
  // Bracket-error scan drives the red border on the outer wrapper
  // (RichTa's textarea itself is border:none — the outer card owns
  // the visible frame). Same source the badge uses, so signals
  // stay in sync.
  const bracketErrors = useMemo(() => scanBracketErrors(value), [value]);
  const hasErr = bracketErrors.length > 0;
  ...
  return (
    <div style={{ border: `1.5px solid ${hasErr ? "rgba(239,68,68,0.8)" : (f ? C.accent : C.border2)}`, borderRadius: 8, overflow: "hidden" }}>
...
export default function ComposeTab({ showToast }: { showToast: (msg: string, type?: string) => void }) {
  ...
  // in the exam and writes them to a 2-sheet workbook with empty
  // columns for the curator to fill in (reading, meaning, notes).
  // Recursively walks data so we don't have to know each question
  // type's shape.
  const handleExportVocabGrammar = async () => {
    if (!exam || questions.length === 0) return;
    setExportingVocab(true);
    try {
      const XLSX = await loadXLSX();
      // Both new (〖〗 / 〔〕) and legacy (【】) brackets are
      // recognised so older exams still surface their tagged words.
      const VOCAB_RE   = /[〖【]([^〗】]+)[〗】]/g;
      const GRAMMAR_RE = /〔([^〕]+)〕/g;
      ...
      const visit = (v: unknown) => {
        ...
        if (typeof v === "string") {
          ...
          while ((m = VOCAB_RE.exec(v)) !== null) {
            const s = m[1].trim();
            if (s) vocab.add(s);
          }
          GRAMMAR_RE.lastIndex = 0;
          while ((m = GRAMMAR_RE.exec(v)) !== null) {
            const s = m[1].trim();
            if (s) grammar.add(s);
          }
          return;
      ...
      for (const q of questions) {
        visit(q);
        // audio_script is a JSON-stringified array — visiting the
        // string above already runs the regexes against the JSON
        // body, so the ぢ inside `text:"…"` cells gets picked up.
      }

      const vocabList   = Array.from(vocab).sort();
      const grammarList = Array.from(grammar).sort();
      ...
      if (vocabList.length > 0) {
        const aoa: (string | number)[][] = [
          ["STT", "Từ vựng", "Đọc", "Nghĩa", "Ghi chú"],
          ...vocabList.map((w, i) => [i + 1, w, "", "", ""] as (string | number)[]),
        ];
        const ws = XLSX.utils.aoa_to_sheet(aoa);
        ws["!cols"] = [{ wch: 5 }, { wch: 22 }, { wch: 16 }, { wch: 32 }, { wch: 22 }];
        XLSX.utils.book_append_sheet(wb, ws, "〖〗 Từ vựng");
      }
      if (grammarList.length > 0) {
        const aoa: (string | number)[][] = [
          ["STT", "Ngữ pháp", "Ý nghĩa", "Ví dụ", "Ghi chú"],
          ...grammarList.map((g, i) => [i + 1, g, "", "", ""] as (string | number)[]),
        ];
        const ws = XLSX.utils.aoa_to_sheet(aoa);
        ws["!cols"] = [{ wch: 5 }, { wch: 22 }, { wch: 32 }, { wch: 42 }, { wch: 22 }];
        XLSX.utils.book_append_sheet(wb, ws, "〔〕 Ngữ pháp");
      }

      const safeName = String(exam.name || "exam").replace(/[\\/?*[\]:]/g, "_").slice(0, 80);
      XLSX.writeFile(wb, `${safeName}_${exam.level || ""}_vocab_grammar.xlsx`);
      showToast(`Export xong: ${vocabList.length} từ vựng · ${grammarList.length} ngữ pháp`, "success");
    } catch (err) {
      console.error("[ExportVocabGrammar]", err);
      ...
      <div style={{ padding: "10px 20px", borderBottom: `1px solid ${C.border}`, background: C.surface, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
          ...
          {questions.length>0 && (
            <button
              type="button"
              className="compose-btn compose-btn-vocab-export"
              onClick={handleExportVocabGrammar}
              disabled={exportingVocab}
